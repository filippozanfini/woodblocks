...
public class DraggableNodeL90 extends DraggableNode {
    ...
    public DraggableNodeL90() {
        ...
        this.setScore(4);
        this.setView(new ImageView(new Image("/assets/Blocks/"+"L90"+".png", 150, 150, false, false)));
    ...
    public boolean setColor(GridPane gameMatrix, boolean conferma) {
        ...
        if(!checkAvailability(x, y) || !checkAvailability(x+1, y) || !checkAvailability(x+2, y) || !checkAvailability(x, y+1)) {
        ...
        if(conferma) {
            rimuoviAnteprima(gameMatrix);
            if(GameMatrix.checkAvailability(x, y) && GameMatrix.checkAvailability(x+1, y) && GameMatrix.checkAvailability(x+2, y) && GameMatrix.checkAvailability(x, y+1)) {
    ...
    private void mostraAnteprima(GridPane gameMatrix, int x, int y) {
        ...
        gameMatrix.add(rectangle4,x, y+1);
    }
