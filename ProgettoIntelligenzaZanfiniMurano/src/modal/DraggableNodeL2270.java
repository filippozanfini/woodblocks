...
public class DraggableNodeL2270 extends DraggableNode {
    ...
    public DraggableNodeL2270() {
        ...
        this.setScore(5);
        this.setView(new ImageView(new Image("/assets/Blocks/"+"L2_270"+".png", 150, 150, false, false)));
    ...
    public boolean setColor(GridPane gameMatrix, boolean conferma) {
        ...
        if(!checkAvailability(x, y) || !checkAvailability(x+1, y) || !checkAvailability(x+2, y) || !checkAvailability(x+2, y-1) || !checkAvailability(x+2, y-2)) {
        ...
        if(conferma) {
            rimuoviAnteprima(gameMatrix);
            if(GameMatrix.checkAvailability(x, y) && GameMatrix.checkAvailability(x+1, y) && GameMatrix.checkAvailability(x+2, y) && GameMatrix.checkAvailability(x, y-1) && GameMatrix.checkAvailability(x+2, y-2)) {
                ...
                if(gameMatrix.getChildren().contains(rectangle1) && gameMatrix.getChildren().contains(rectangle2) &&
                    ...
                    gameMatrix.getChildren().contains(rectangle5)){
                    rimuoviAnteprima(gameMatrix);
    ...
    private void mostraAnteprima(GridPane gameMatrix, int x, int y) {
        ...
        gameMatrix.add(rectangle4,x+2, y-1);
    ...
    private void aggiungiBlocco(GridPane gameMatrix, int x, int y) {
        ...
        GameMatrix.add(x+2, y-1);
        GameMatrix.add(x+2, y-2);
